...
ACMD_DECL(do_inquisitor_favored_enemy);
...
ACMD_DECL(do_reckless_abandon);
ACMD_DECL(do_warcry);
ACMD_DECL(do_earthshaker);
ACMD_DECL(do_hardy);
...
ACMD_DECL(do_fistair);
ACMDCHECK(can_fistair);
ACMD_DECL(do_flowingriver);
...
ACMD_DECL(do_clenchofnorthwind);
...
ACMD_DECL(do_smokebomb);
ACMDCHECK(can_smokebomb);
ACMD_DECL(do_miststance);
...
ACMD_DECL(do_summonerpreference);

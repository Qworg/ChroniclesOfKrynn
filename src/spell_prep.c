...
#define IS_SPELL(spellnum)                                                                         \
  ((spellnum) > 0 && (spellnum) < TOP_SPELL_DEFINE && (spellnum) < SKILL_BACKSTAB)
 ...
 */
void prep_queue_remove(struct char_data *ch, struct prep_collection_spell_data *entry, int class)
{
 ...
 */
void innate_magic_remove(struct char_data *ch, struct innate_magic_data *entry, int class)
{
 ...
 */
void collection_remove(struct char_data *ch, struct prep_collection_spell_data *entry, int class)
{
  struct prep_collection_spell_data *temp;
  ...
  {
    send_to_char(ch,
                 "{entered collection_remove(), variables: caster: %s, spellnum: %d, domain: %d, "
                 "prep-time: %d, meta-magic: %d}    ",
 ...
 */
void known_spells_remove(struct char_data *ch, struct known_spell_data *entry, int class)
{
 ...
 */
void prep_queue_add(struct char_data *ch, int ch_class, int spellnum, int metamagic, int prep_time,
                    int domain)
{
 ...
 */
void innate_magic_add(struct char_data *ch, int ch_class, int circle, int metamagic, int prep_time,
                      int domain)
{
 ...
 */
void collection_add(struct char_data *ch, int ch_class, int spellnum, int metamagic, int prep_time,
                    int domain)
{
  struct prep_collection_spell_data *entry;
  struct prep_collection_spell_data *current;
...
bool known_spells_add(struct char_data *ch, int ch_class, int spellnum, bool loading)
{
  ...
  if (!loading)
  {
    ...
    case CLASS_BARD:
      ...
      if (bard_known[caster_level][circle] - count_known_spells_by_circle(ch, ch_class, circle) <=
          0)
        return FALSE;
      break;
    case CLASS_SUMMONER:
      /* Check against summoner spells known table */
      if (summoner_known[caster_level][circle] -
    ...
    case CLASS_INQUISITOR:
      ...
      if (inquisitor_known[caster_level][circle] -
              count_known_spells_by_circle(ch, ch_class, circle) <=
          0)
        return FALSE;
      break;
...
int count_total_slots(struct char_data *ch, int class, int circle)
{
  ...
  total_slots =
      (count_circle_collection(ch, class, circle) + count_circle_innate_magic(ch, class, circle) +
 ...
 */
int is_spell_in_prep_queue(struct char_data *ch, int class, int spellnum, int metamagic)
{
  struct prep_collection_spell_data *current = SPELL_PREP_QUEUE(ch, class);
  struct prep_collection_spell_data *next;
 ...
 */
bool is_spell_in_collection(struct char_data *ch, int class, int spellnum, int metamagic)
{
  struct prep_collection_spell_data *current = SPELL_COLLECTION(ch, class);
  struct prep_collection_spell_data *next;
...
static int level_to_circle_conversion(int min_level, int caster_type)
{
  ...
  case 1: /* 3/4 caster (Bard, Alchemist, etc) - detailed progression */
    if (min_level <= 3)
      ...
      return 3;
    else if (min_level <= 12)
      return 4;
    else if (min_level <= 15)
      return 5;
    else if (min_level <= 20)
      return 6;
    else
  ...
  case 2: /* Half caster (Paladin, Ranger, etc) */
    if (min_level <= 9)
      return 1;
    else if (min_level <= 11)
      return 2;
    else if (min_level <= 14)
      return 3;
    else if (min_level <= 20)
      return 4;
    else
 ...
 */
int compute_spells_circle(struct char_data *ch, int char_class, int spellnum, int metamagic,
                          int domain)
{
  ...
  case CLASS_WARLOCK:
    ...
    metamagic_mod =
        apply_automatic_metamagic_reduction(ch, metamagic, metamagic_mod, spell_circle, 4);
...
void start_prep_event(struct char_data *ch, int class)
{
  ...
  case CLASS_SUMMONER:
    ...
    {
      send_to_char(ch,
                   "You have nothing in your innate magic queue for this class to prepare!\r\n");
      return;
    }
    break;
  default:
...
bool is_min_level_for_spell(struct char_data *ch, int class, int spellnum)
{
  ...
  case CLASS_CLERIC: /* Domain system - check both domains for best access */
    ...
    if ((BONUS_CASTER_LEVEL(ch, class) + CLASS_LEVEL(ch, CLASS_CLERIC)) < min_level)
    {
      return FALSE;
    }
    break;
...
int spell_prep_gen_extract(struct char_data *ch, int spellnum, int metamagic)
{
  ...
  if (!IS_NPC(ch) && has_moon_bonus_spells(ch))
  {
    ...
    for (ch_class = 0; ch_class < NUM_CLASSES; ch_class++)
    {
      ...
      {
        is_arcane_spell =
            (ch_class == CLASS_WIZARD || ch_class == CLASS_SORCERER || ch_class == CLASS_BARD ||
             ch_class == CLASS_SUMMONER || ch_class == CLASS_ARCANE_SHADOW ||
             ch_class == CLASS_KNIGHT_OF_THE_THORN || ch_class == CLASS_ELDRITCH_KNIGHT ||
             ch_class == CLASS_ARCANE_ARCHER || ch_class == CLASS_SPELLSWORD ||
  ...
  for (ch_class = 0; ch_class < NUM_CLASSES; ch_class++)
  {
    ...
    if (is_spell_in_collection(ch, ch_class, spellnum, check_metamagic))
    {
      ...
      if (is_arcane_class && check_spell_slot_preservation(ch))
      {
        send_to_char(
            ch,
            "\tW[Your mastery of magic allows you to cast without expending the spell!]\tn\r\n");
  ...
  for (ch_class = 0; ch_class < NUM_CLASSES; ch_class++)
  {
         ...
         0))
    {
      ...
      if (is_arcane_class && check_spell_slot_preservation(ch))
      {
        send_to_char(ch, "\tW[Your mastery of magic allows you to cast without expending the spell "
                         "slot!]\tn\r\n");
...
int spell_prep_gen_check(struct char_data *ch, int spellnum, int metamagic)
{
  ...
  for (class = 0; class < NUM_CLASSES; class ++)
  {
    ...
    if (CLASS_LEVEL(ch, CLASS_INQUISITOR) && class == CLASS_INQUISITOR)
      circle_of_this_spell =
          compute_spells_circle(ch, class, spellnum, metamagic, GET_1ST_DOMAIN(ch));
    else
      circle_of_this_spell =
          compute_spells_circle(ch, class, spellnum, metamagic, DOMAIN_UNDEFINED);
...
void print_prep_queue(struct char_data *ch, int ch_class)
{
  ...
  *buf = '\0';
  for (; current; current = next)
  {
    next = current->next;
    int spell_circle =
        compute_spells_circle(ch, ch_class, current->spell, current->metamagic, current->domain);
    int prep_time = current->prep_time;
    total_time += prep_time;
    send_to_char(
        ch, " \tW%20s\tn \tc[\tn%d%s circle\tc]\tn \tc[\tn%2d seconds\tc]\tn %s%s%s%s%s%s %s\r\n",
        spell_info[current->spell].name, spell_circle,
        (spell_circle == 1)   ? "st"
        : (spell_circle == 2) ? "nd"
        : (spell_circle == 3) ? "rd"
                              : "th",
        prep_time, (IS_SET(current->metamagic, METAMAGIC_QUICKEN) ? "\tc[\tnquickened\tc]\tn" : ""),
  ...
  *buf = '\0';
  ...
  if (ch_class == CLASS_WIZARD)
    send_to_char(ch,
                 "Wizards must have their spellbook in their inventory to memorize spells.\r\n");
...
void print_innate_magic_queue(struct char_data *ch, int ch_class)
{
  ...
  *buf = '\0';
  for (; current; current = next)
  {
    ...
    total_time += prep_time;
    send_to_char(
        ch, " \tc[\tWcircle-slot: \tn%d%s\tc]\tn \tc[\tn%2d seconds\tc]\tn %s%s%s%s%s%s %s\r\n",
        current->circle,
        (current->circle == 1)   ? "st"
        : (current->circle == 2) ? "nd"
        : (current->circle == 3) ? "rd"
...
static struct spell_count_data *find_or_create_spell_count(struct spell_count_data **spell_counts,
                                                           int circle, int spell, int metamagic,
                                                           int domain)
{
  ...
  while (count_entry)
  {
    if (count_entry->spell == spell && count_entry->metamagic == metamagic &&
                                    ...
                                    struct spell_count_data **spell_counts)
{
  ...
  for (current = SPELL_COLLECTION(ch, ch_class); current; current = current->next)
  {
    ...
    {
      find_or_create_spell_count(spell_counts, this_circle, current->spell, current->metamagic,
                                 current->domain);
    }
  }
}
                                  ...
                                  struct spell_count_data *spell_list)
{
  ...
  while (count_entry)
  {
    ...
    build_metamagic_string(metamagic_str, sizeof(metamagic_str), count_entry->metamagic,
                           count_entry->domain);
    ...
    if (counter == 1)
    {
      send_to_char(ch, "\tY%d%s:\tn    \tW%s\tn%s\r\n", circle, get_ordinal_suffix(circle),
                   spell_info[count_entry->spell].name, metamagic_str);
    }
    else
    {
...
void print_collection(struct char_data *ch, int ch_class)
{
  ...
  /* build a nice heading */
  *buf = '\0';
  snprintf(buf, sizeof(buf), "\tY%s Collection for %s\tC",
           ch_class == CLASS_ALCHEMIST ? "Extract" : "Spell", class_names[ch_class]);
  send_to_char(ch, "\tC");
  text_line(ch, buf, line_length, '-', '-');
  send_to_char(ch, "\tn");
...
void display_available_slots(struct char_data *ch, int class)
{
  ...
  for (slot = 0; slot <= highest_circle; slot++)
  {
    ...
    if ((slot_array[slot] =
             compute_slots_by_circle(ch, class, slot) - count_total_slots(ch, class, slot)) > 0)
      found_slot = TRUE;
...
void reset_preparation_time(struct char_data *ch, int class)
{
  ...
  default:
    ...
    preparation_time = compute_spells_prep_time(
        ch, class,
        compute_spells_circle(ch, class, SPELL_PREP_QUEUE(ch, class)->spell,
...
int star_circlet_proc(struct char_data *ch, int num_times)
{
  ...
  do
  {
    ...
    if (which_class != -1)
    {
      ...
      default:
        if (SPELL_PREP_QUEUE(ch, class))
        {
          ...
          collection_add(
              ch, class, SPELL_PREP_QUEUE(ch, class)->spell, SPELL_PREP_QUEUE(ch, class)->metamagic,
              SPELL_PREP_QUEUE(ch, class)->prep_time, SPELL_PREP_QUEUE(ch, class)->domain);
    ...
    if (!found_spell_this_iteration)
    {
      ...
      if (empty_iterations >= MAX_EMPTY_ITERATIONS)
      {
        ...
        {
          send_to_char(ch,
                       "DEBUG: star_circlet_proc - Exiting early after %d empty iterations.\r\n",
                       empty_iterations);
        }
  ...
  if (DEBUGMODE)
  {
    ...
    send_to_char(ch,
                 "DEBUG: star_circlet_proc completed in %ld microseconds (%d iterations, %d spells "
                 "processed).\r\n",
...
static bool validate_preparation_queue(struct char_data *ch, int class)
{
  ...
  case CLASS_SUMMONER:
    ...
    if (!INNATE_MAGIC(ch, class))
    {
      send_to_char(
          ch,
          "Your preparations are aborted!  You do not seem to have anything in your queue!\r\n");
      return FALSE;
    }
    break;
  default:
...
static int process_prepared_caster_prep(struct char_data *ch, int class)
{
  ...
  if (SPELL_PREP_QUEUE(ch, class)->prep_time <= 0)
  {
    ...
    collection_add(ch, class, SPELL_PREP_QUEUE(ch, class)->spell,
                   SPELL_PREP_QUEUE(ch, class)->metamagic, SPELL_PREP_QUEUE(ch, class)->prep_time,
                   SPELL_PREP_QUEUE(ch, class)->domain);
...
ACMDU(do_consign_to_oblivion)
{
  ...
  *arg = '\0';
  ...
  default:
    ...
    log("SYSERR: do_consign_to_oblivion() called with invalid subcmd %d by %s", subcmd,
        GET_NAME(ch));
    return;
  }
  ...
  if (class < 0 || class >= NUM_CLASSES)
  {
    ...
    log("SYSERR: do_consign_to_oblivion() calculated invalid class %d for subcmd %d by %s", class,
        subcmd, GET_NAME(ch));
    return;
  }
  ...
  else
  {
    ...
    {
      send_to_char(ch,
                   "There is nothing in your preparation queue or %s "
                   "collection to %s!\r\n",
                   class == CLASS_ALCHEMIST ? "extract" : "spell", spell_consign_dict[class][0]);
      stop_prep_event(ch, class);
      return;
  ...
  if (!IS_SPELL(spellnum))
  {
    ...
    log("SYSERR: do_consign_to_oblivion() - %s tried to forget non-spell %d (%s)", GET_NAME(ch),
        spellnum, spell_info[spellnum].name);
    return;
  }
  ...
  if (metamagic != 0)
  {
    ...
    if (metamagic & ~valid_metamagic)
    {
      ...
      log("SYSERR: do_consign_to_oblivion() - %s used invalid metamagic flags %d", GET_NAME(ch),
          metamagic);
      return;
    }
  }
  ...
  if (SPELL_PREP_QUEUE(ch, class))
  {
    ...
    if (SPELL_PREP_QUEUE(ch, class)->spell == spellnum &&
        SPELL_PREP_QUEUE(ch, class)->metamagic == metamagic && PREPARING_STATE(ch, class))
    {
      send_to_char(ch,
                   "Being that %s was the next in your preparation queue, you are "
  ...
  send_to_char(ch,
               "You do not have %s in your preparation queue or %s collection! "
               "(make sure you used the correct command based on class and you used the "
               "proper meta-magic arguments)\r\n",
               spell_info[spellnum].name, class == CLASS_ALCHEMIST ? "extract" : "spell");
}
...
ACMDU(do_gen_preparation)
{
  ...
  default:
    send_to_char(ch, "Invalid command!\r\n");
    log("SYSERR: do_gen_preparation() called with invalid subcmd %d by %s", subcmd, GET_NAME(ch));
    return;
  }
  ...
  {
    send_to_char(ch, "Invalid class detected!\r\n");
    log("SYSERR: do_gen_preparation() calculated invalid class %d for subcmd %d by %s", class,
        subcmd, GET_NAME(ch));
    return;
  }
  ...
  if (!get_class_highest_circle(ch, class))
  {
    send_to_char(ch, "Try changing professions (type score to view respective preparation commands "
                     "for your class(es)!\r\n");
    return;
  }
  ...
  if (!IS_SPELL(spellnum))
  {
    ...
    log("SYSERR: do_gen_preparation() - %s tried to prepare non-spell %d (%s)", GET_NAME(ch),
        spellnum, spell_info[spellnum].name);
    return;
  }
  ...
  if (metamagic != 0)
  {
    ...
    if (metamagic & ~valid_metamagic)
    {
      ...
      log("SYSERR: do_gen_preparation() - %s used invalid metamagic flags %d", GET_NAME(ch),
          metamagic);
      return;
    }
...
int class_to_spell_prep_scmd(int class_name)
{
  ...
  {
  case CLASS_WIZARD:
    return SCMD_MEMORIZE;
  case CLASS_CLERIC:
    return SCMD_PRAY;
  case CLASS_DRUID:
    return SCMD_COMMUNE;
  case CLASS_SORCERER:
    return SCMD_MEDITATE;
  case CLASS_PALADIN:
    return SCMD_CHANT;
  case CLASS_RANGER:
    return SCMD_ADJURE;
